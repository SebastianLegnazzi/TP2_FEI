...
function easy_slider_revolution_shortcode( $atts ) {
	...
	wp_register_style( 'owl_carousel_css', ESRCPT_PLUGIN_PATH . 'owl-carousel/owl.carousel.css', array(), '2.2.1.1', 'all' );
	wp_enqueue_style( 'owl_carousel_css' );
	wp_register_style( 'owl_theme_css', ESRCPT_PLUGIN_PATH . 'owl-carousel/sa-owl-theme.css', array(), '2.0', 'all' );
	wp_enqueue_style( 'owl_theme_css' );
	wp_register_style( 'owl_animate_css', ESRCPT_PLUGIN_PATH . 'owl-carousel/animate.min.css', array(), '2.0', 'all' );
	wp_enqueue_style( 'owl_animate_css' );
	wp_register_script( 'mousewheel_js', ESRCPT_PLUGIN_PATH . 'js/jquery.mousewheel.min.js', array( 'jquery' ), '3.1.13', true );
	wp_enqueue_script( 'mousewheel_js' );
	wp_register_script( 'owl_thumbs_js', ESRCPT_PLUGIN_PATH . 'owl-carousel/owl.carousel2.thumbs.min.js', array( 'jquery' ), '0.1.8', true );
	wp_enqueue_script( 'owl_thumbs_js' );
	// JAVASCRIPT/CSS FOR LIGHTGALLERY (POPUP) LIBRARY (lightgalleryjs.com).
	wp_register_script( 'lightgallery_js', ESRCPT_PLUGIN_PATH . 'lightgallery/lightgallery.min.js', array( 'jquery' ), '2.5.0', true );
	wp_enqueue_script( 'lightgallery_js' );
	wp_register_script( 'lightgallery_video_js', ESRCPT_PLUGIN_PATH . 'lightgallery/plugins/video/lg-video.min.js', array( 'jquery' ), '2.5.0', true );
	wp_enqueue_script( 'lightgallery_video_js' );
	wp_register_script( 'lightgallery_zoom_js', ESRCPT_PLUGIN_PATH . 'lightgallery/plugins/zoom/lg-zoom.min.js', array( 'jquery' ), '2.5.0', true );
	wp_enqueue_script( 'lightgallery_zoom_js' );
	wp_register_script( 'lightgallery_autoplay_js', ESRCPT_PLUGIN_PATH . 'lightgallery/plugins/autoplay/lg-autoplay.min.js', array( 'jquery' ), '2.5.0', true );
	wp_enqueue_script( 'lightgallery_autoplay_js' );
	wp_register_script( 'vimeo_player_js', ESRCPT_PLUGIN_PATH . 'lightgallery/player.min.js', array( 'jquery' ), '2.17.1', true );
	wp_enqueue_script( 'vimeo_player_js' );
	wp_register_style( 'lightgallery_css', ESRCPT_PLUGIN_PATH . 'lightgallery/css/lightgallery.css', array(), '2.5.0', 'all' );
	wp_enqueue_style( 'lightgallery_css' );
	wp_register_style( 'lightgallery_bundle_css', ESRCPT_PLUGIN_PATH . 'lightgallery/css/lightgallery-bundle.min.css', array(), '2.5.0', 'all' );
	...
	if ( 0 === $id ) {
		...
		$output .= "<div id='sa_invalid_postid'>EASY SLIDER shortcode error: A valid ID has not been provided</div>\n";
	} else {
		...
		if ( ( 'publish' !== $post_status ) || ( 0 === count( $metadata ) ) || ( 'es_slider' !== $post_type ) ) {
			// SHORTCODE 'id' PARAMETER PROVIDED IS INVALID.
			$output .= "<div id='sa_invalid_postid'>Image Slider shortcode error: A valid ID has not been provided</div>\n";
		} else {
			...
			for ( $i = 1; $i <= $slide_data['num_slides']; $i++ ) {
				...
				$slide_data[ 'slide' . $i . '_link_text' ]    = $metadata[ 'sa_slide' . $i . '_link_text' ][0];
				$slide_data[ 'slide' . $i . '_button_background' ]    = $metadata[ 'sa_slide' . $i . '_button_background' ][0];
				$slide_data[ 'slide' . $i . '_button_color' ]    = $metadata[ 'sa_slide' . $i . '_button_color' ][0];
			...
			$slide_data['items_width1'] = 1;
			$slide_data['items_width2'] = 1;
			$slide_data['items_width3'] = 1;
			$slide_data['items_width4'] = 1;
			$slide_data['items_width5'] = 1;
			$slide_data['items_width6'] = 1;
			...
			for ( $i = 1; $i <= $slide_data['num_slides']; $i++ ) {
				...
				$slide_style        = 'padding:' . $slide_data['slide_padding_tb'] . 'px ' . $slide_data['slide_padding_lr'] . 'px; ';
				...
				if ( strpos( $slide_data['slide_min_height_perc'], 'px' ) !== false ) {
					$slide_style .= 'height:' . $slide_data['slide_min_height_perc'] . '; ';
				}
				...
				if ( ( 'IMAGE' === $slide_data[ 'slide' . $i . '_popup_type' ] ) && ( '' !== $slide_data[ 'slide' . $i . '_popup_image' ] ) ) {
					$lightbox_count++;
					$popup_output = "<div class='sa_popup_zoom_icon' onClick=' esc_js(" . $lightbox_function . '(' . $lightbox_count . "))'></div>";
				}
				if ( ( 'VIDEO' === $slide_data[ 'slide' . $i . '_popup_type' ] ) && ( '' !== $slide_data[ 'slide' . $i . '_popup_video_id' ] ) ) {
					$lightbox_count++;
					$popup_output = "<div class='sa_popup_video_icon' onClick=' esc_js(" . $lightbox_function . '(' . $lightbox_count . "))'></div>";
				}
				if ( 'HTML' === $slide_data[ 'slide' . $i . '_popup_type' ] ) {
					$lightbox_count++;
					$popup_css_id = $slide_data[ 'slide' . $i . '_popup_css_id' ];
					$popup_output = "<div class='sa_popup_zoom_icon' onClick=' esc_js(document.getElementById(\"" . $popup_css_id . "\").style.display=\"block\")'></div>";
				}
				...
				for ( $i = 1; $i <= $slide_data['num_slides']; $i++ ) {
					...
					if ( ( 'IMAGE' === $slide_data[ 'slide' . $i . '_popup_type' ] ) &&
						...
						if ( $thumb_image_src ) {
							...
							$thumb_image_src = ESRCPT_PLUGIN_PATH . 'images/image_placeholder.jpg';
						}
					} else {
			...
			for ( $i = 1; $i <= $slide_data['num_slides']; $i++ ) {
				if ( 'HTML' === $slide_data[ 'slide' . $i . '_popup_type' ] ) {
					...
					$output      .= "<div id='" . $popup_css_id . "' class='sa_custom_popup' onClick=' esc_js(this.style.display = \"none\")'>\n";
					$output      .= "<div class='sa_popup_close' onClick=' esc_js(document.getElementById(\"" . $popup_css_id . "\").style.display=\"none\")'>X</div>";
					$output      .= "<div class='sa_popup_wrap' style='background:" . $popup_bgcol . '; max-width:' . $popup_width . "px;' onclick=' esc_js(event.stopPropagation())'>\n";
			...
			if ( $lightbox_count > 0 ) {
				...
				for ( $i = 1; $i <= $slide_data['num_slides']; $i++ ) {
					...
					if ( ( 'IMAGE' === $slide_data[ 'slide' . $i . '_popup_type' ] ) && ( '' !== $slide_data[ 'slide' . $i . '_popup_image' ] ) ) {
						...
						$img_url     = esc_url($slide_data[ 'slide' . $i . '_popup_image' ]);
			...
			if ( '1' !== $slide_data['hero_slider'] ) {
				...
				if ( ( '' !== $slide_min_height ) && ( '0' !== $slide_min_height ) ) {
					...
					$output .= '				var slide_width = slider_width / ' . 480 . ";\n";
					$output .= "			} else if (win_width < 768) {\n";
					$output .= '				var slide_width = slider_width / ' . 768 . ";\n";
					$output .= "			} else if (win_width < 980) {\n";
					$output .= '				var slide_width = slider_width / ' . 980 . ";\n";
					$output .= "			} else if (win_width < 1200) {\n";
					$output .= '				var slide_width = slider_width / ' . 1200 . ";\n";
					$output .= "			} else if (win_width < 1500) {\n";
					$output .= '				var slide_width = slider_width / ' . 1500 . ";\n";
					$output .= "			} else {\n";
					$output .= '				var slide_width = slider_width / ' . 1800 . ";\n";
					$output .= "			}\n";
					...
					$output .= "			var slide_height = '".$slide_data['slide_min_height_perc']."';\n";
					...
					$output .= "				slide_height = Math.round(slide_height)+'px';\n";
					$output .= "			} else {\n";
					$output .= '				slide_height = slide_height;';
					$output .= "				slide_height = Math.round(slide_height)+'px';\n";
					$output .= "			}\n";
